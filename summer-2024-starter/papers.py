...
TEST_FIRST_LINE = 'first_line.csv'
...
class PaperTree(TMTree):
    ...
    def __init__(self, name: str, subtrees: List[TMTree], authors: str = '',
        ...
        if all_papers:
            paper_info_dict = _load_papers_to_dict(by_year)
            self._subtrees.extend(_build_tree_from_dict(paper_info_dict))
            self.data_size = sum(
                subtree.data_size for subtree in self._subtrees)

            for subt in self._subtrees:
                subt._parent_tree = self
...
def _load_papers_to_dict(by_year: bool = True) -> Dict:
    ...
    with open(DATA_FILE, newline='') as file:
        ...
        for row in reader:
            # current level of the dictionary we are on
            current_level = paper_dict

            # if by_year is true, then set the year
            # to be the first key of the dictionary
            if by_year:
                year = row['Year']
                if year not in current_level:
                    current_level[year] = {}
                current_level = current_level[year]

            categories = row['Category'].split(':')

            # now add all the categories as keys/values
            for category in categories:
                if category.strip() not in current_level:
                    current_level[category.strip()] = {}
                current_level = current_level[category.strip()]

            # once you get to the last category, add this key
            # that points to a list containing all the papers in this category
            if 'actual cs papers FOR ASSIGNMENT' not in current_level:
                current_level['actual cs papers FOR ASSIGNMENT'] = []

            # add the paper details of the row in the list above^^
            paper_to_add = {
                'title': row['Title'],
                'author': row['Author'],
                'url': row['Url'],
            ...
            current_level['actual cs papers FOR ASSIGNMENT'].append(
                paper_to_add)
...
def _build_tree_from_dict(nested_dict: Dict) -> List[PaperTree]:
    ...
    """
    if list(nested_dict.keys()) == []:
        return []

    trees = []

    surface_dict_items = list(nested_dict.keys())

    for subtree in surface_dict_items:
        if subtree == 'actual cs papers FOR ASSIGNMENT':
            for paper in nested_dict[subtree]:
                paper_subtree = PaperTree(
                    paper['title'],
                    [],
                    paper['author'],
                    paper['url'],
                    paper['citation'])
                trees.append(paper_subtree)
        else:
            subtrees_of_category = (
                _build_tree_from_dict(nested_dict[subtree]))
            category_subtree = PaperTree(subtree, subtrees_of_category)

            category_subtree.update_rectangles(category_subtree.rect)
            category_subtree.update_data_sizes()

            trees.append(category_subtree)
